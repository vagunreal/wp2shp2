...
- **本程序不是本仓库上传者开发的**。本工具来自一个**百度网盘公开分享链接**下载到的
  分发版二进制（`wp2shp2.exe`，约 200MB，PyInstaller + PyArmor 打包的 Python 程序）。
  上传者**不知道原作者是谁**，也无法联系到对方，更从未参与过该软件的开发或销售。
- 本仓库是上传者对该二进制进行**反向还原（逆向分析）**后重写的可编译源码：
  原版以 PyArmor 加密，本仓库从字节码逐函数还原，行为与原版一致。
- 原版界面与试用文案中出现的**联系人与微信号已全部删除**，本仓库不含任何
  原作者的个人信息与联系方式；请勿在任何场景冒充作者或声称拥有本软件版权。
- 原版含有授权限制（试用次数、个人版仅支持 SHP 等），本开源版本**已移除全部
  授权检查**，是**完全免费**的转换工具：把 MapGIS 的 `.wt`（点）/ `.wp`（区）/
  `.wl`（线）文件免费转换成 `.shp`（ESRI Shapefile）等通用矢量格式。
- 如原作者看到本仓库并希望主张权利、下架或署名，欢迎在本仓库提 issue 说明，
  上传者会配合处理。
